...
export default function EditSupplierPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const loadSupplier = async () => {
      setIsLoading(true);
      try {
        const data = (await supplierService.getSupplierById(params.id)).data;
        setFormData({
          name: data.name || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await supplierService.updateSupplier(params.id, formData);
  ...
  if (isLoading) {
    return (
      <div className="p-6 flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <p className="text-muted-foreground">Cargando datos del proveedor...</p>
      </div>
    );
  }
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center gap-2">
        <Button
          variant="ghost"
          size="icon"
          onClick={() => router.push(`/dashboard/suppliers/${params.id}`)}
        >
          <ArrowLeft size={18} />
        </Button>
